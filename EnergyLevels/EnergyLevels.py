...
width_spacing_f = float(data[theline+8]) # Spacing factor for width boxes
...
for k in range(1,cols+1):
    ...
    for i in range(0,data_len):
        ...
        jj = line.split()[3]
        parity = int(line.split()[4])
        if parity == 0:
            pp = '-'
        elif parity == 1:
            pp = '+'
        index = int(line.split()[5])
        jpi_pos = line.split()[6]
        bttm = line.split()[7]
        if bttm == 'None':
            bttm = ''
        top = line.split()[8]
        ...
        if len(line.split()) > 9:
            ...
            link_style = '--'
            link_width = 0.75
            n_add = len( line.split()[9::] )
            for j in range(0,n_add):
                if line.split()[9+j].split('#')[0] == 'C': # Specific color line
                    level_color = int(line.split()[9+j].split('#')[1])
                if line.split()[9+j].split('#')[0] == 'LS': # Specific line style
                    level_ls = line.split()[9+j].split('#')[1]
                # Possible linking information
                if line.split()[9+j].split('#')[0] == 'L': # Set link destination
                    link_out = int(line.split()[9+j].split('#')[1])
                    link_set = 1
                if line.split()[9+j].split('#')[0] == 'LC': # Set link color
                    link_color = int(line.split()[9+j].split('#')[1])
                if line.split()[9+j].split('#')[0] == 'LLS': # Set link style
                    link_style = line.split()[9+j].split('#')[1]
                if line.split()[9+j].split('#')[0] == 'LLW': # Set link width
                    link_width = float(line.split()[9+j].split('#')[1])
...
for box in width_boxes_data:
    level_id = box[0]
    width = box[1]
    figure.add_widthbox(level_id, width, width_spacing_f)
# Linking
...
OUTPUT FILE # 1 - Name of the figure, 2- Format: png, jpg, eps, pdj, pgf (xenex package needed), 3- Fig dpi resolution, 4- 'x,y' Figsize in pixel, 5- (m,M) minor and major tick.space, 6- Control the distance between plot and y_axes (0 to 1), 7- Control the distance between plot and x_axes (0 to 1), 8- Control the distance of the width-box with the state line end
CC_11C_Spectrum
pdf
200
1200,1200
...
# INDEX ENERGY WIDTH  J  PI(0 for '-' and 1 for '+')  INDEX('-1' if not plotting index) JPIPOS('left' or 'right' or 'None') BOTTOMTEXT('None' for nothing) TOPTEXT('None' for nothing) COLOR(optional) LS(optional) LINKINDEX(optional) LINKCOLOR(optional) LINKLS(optional) LINKLW(optional)
  0 -45.145 -1   3/2  0  -1  right  None  None
  1 -43.145 -1   1/2  0  -1  right  None  None
  2 -40.827 -1   5/2  0  -1  right  None  None
  3 -40.341 -1   3/2  0  -1  right  None  None
  4 -38.667 -1   7/2  0  -1  right  None  None
  5 -37.041 -1   3/2  0  -1  right  None  None
  6 -36.725 -1   5/2  0  -1  right  None  None
  7 -35.500 -1   3/2  0  -1  right  None  None
  8 -35.365 -1   5/2  0  -1  right  None  None
  9 -35.175 -1   7/2  0  -1  right  None  None
 10 -37.602 -1   -1   0  -1  None   None  None   C#1 LS#--
 11 -36.456 -1   -1   0  -1  None   None  None   C#1 LS#--
 12 -35.923 -1   -1   0  -1  None   None  None   C#1 LS#--
...
# Data must have the following columns:
# INDEX ENERGY WIDTH  J  PI(0 for '-' and 1 for '+')  INDEX('-1' if not plotting index) JPIPOS('left' or 'right' or 'None') BOTTOMTEXT('None' for nothing) TOPTEXT('None' for nothing) COLOR(optional) LS(optional) LINKINDEX(optional) LINKCOLOR(optional) LINKLS(optional) LINKLW(optional)
 13 -43.478 -1   3/2  0  -1  None  None  None              L#0 LC#-1 LLS#--
 14 -42.382 -1   1/2  0  -1  None  None  None              L#1 LC#-1 LLS#--
 15 -34.131 -1   1/2  0  -1  left  None  None 
 16 -39.528 -1   5/2  0  -1  None  None  None              L#2 LC#-1 LLS#--
 17 -37.934 -1   3/2  0  -1  None  None  None              L#3 LC#-1 LLS#--
 18 -38.168 -1   7/2  0  -1  None  None  None              L#4 LC#-1 LLS#--
 19 -35.222 -1   3/2  0  -1  None  None  None              L#5 LC#-1 LLS#--
 20 -35.157 -1   5/2  0  -1  None  None  None              L#6 LC#-1 LLS#--
 21 -32.186 -1   3/2  0  -1  None  None  None              L#7 LC#-1 LLS#--
 22 -32.255 -1   5/2  0  -1  None  None  None              L#8 LC#-1 LLS#--
 23 -37.602 -1   -1   0  -1  None  None  None   C#1 LS#--  L#10 LC#1 LLS#-- LLW#1.5
 24 -36.456 -1   -1   0  -1  None  None  None   C#1 LS#--  L#11 LC#1 LLS#-- LLW#1.5
 25 -35.923 -1   -1   0  -1  None  None  None   C#1 LS#--  L#12 LC#1 LLS#-- LLW#1.5
...
# Data must have the following columns:
# INDEX ENERGY WIDTH  J  PI(0 for '-' and 1 for '+')  INDEX('-1' if not plotting index) JPIPOS('left' or 'right' or 'None') BOTTOMTEXT('None' for nothing) TOPTEXT('None' for nothing) COLOR(optional) LS(optional) LINKINDEX(optional) LINKCOLOR(optional) LINKLS(optional) LINKLW(optional)
 26 -38.806 -1   1/2  1  -1  right  None  None
 27 -38.241 -1   5/2  1  -1  right  None  None
 28 -37.745 -1   3/2  1  -1  right  None  None
 29 -37.646 -1   3/2  1  -1  right  None  None
 30 -36.491 -1   7/2  1  -1  right  None  None
 31 -36.446 -1   5/2  1  -1  right  None  None
 32 -35.945 -1   5/2  1  -1  right  None  None
 33 -35.062 -1   7/2  1  -1  right  None  None
 34 -37.602 -1   -1   0  -1  None   None  None   C#1 LS#--  L#23 LC#1 LLS#-- LLW#1.5
 35 -36.456 -1   -1   0  -1  None   None  None   C#1 LS#--  L#24 LC#1 LLS#-- LLW#1.5
 36 -35.923 -1   -1   0  -1  None   None  None   C#1 LS#--  L#25 LC#1 LLS#-- LLW#1.5
...
# Data must have the following columns:
# INDEX ENERGY WIDTH  J  PI(0 for '-' and 1 for '+')  INDEX('-1' if not plotting index) JPIPOS('left' or 'right' or 'None') BOTTOMTEXT('None' for nothing) TOPTEXT('None' for nothing) COLOR(optional) LS(optional) LINKINDEX(optional) LINKCOLOR(optional) LINKLS(optional) LINKLW(optional)
 37 -39.730 -1   3/2  1  -1  None   None  None                                   L#28 LC#-1 LLS#--
 38 -39.383 -1   3/2  1  -1  None   None  None                                   L#29 LC#-1 LLS#--
 39 -39.805 -1   1/2  1  -1  None   None  None                                   L#26 LC#-1 LLS#--
 40 -39.307 -1   1/2  1  -1  left   None  None
 41 -36.556 -1   1/2  1  -1  left   None  None
 42 -39.808 -1   5/2  1  -1  None   None  None                                   L#27 LC#-1 LLS#--
 43 -35.098 -1   5/2  1  -1  None   None  None                                   L#31 LC#-1 LLS#--
 44 -34.368 -1   5/2  1  -1  None   None  None                                   L#32 LC#-1 LLS#--
 45 -35.009 -1   7/2  1  -1  None   None  None                                   L#30 LC#-1 LLS#--
 46 -33.069 -1   7/2  1  -1  None   None  None                                   L#33 LC#-1 LLS#--
 47 -37.602 -1   -1   0  -1  None   None  $\alpha-th$          C#1 LS#--  L#34 LC#1 LLS#-- LLW#1.5
 48 -36.456 -1   -1   0  -1  None   None  $p-th$               C#1 LS#--  L#35 LC#1 LLS#-- LLW#1.5
 49 -35.923 -1   -1   0  -1  None   None  $^{3}\mathrm{He}-th$ C#1 LS#--  L#36 LC#1 LLS#-- LLW#1.5
_________________________________
"""